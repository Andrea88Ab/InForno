...
namespace InForno.Controllers
{
    ...
    public class OrdineController : Controller
    {
        ...
        public ActionResult AddOrdine(Ordine ordine)
        {
            ...
            {
                TempData["SuccessMessage"] = "Ordine effettuato con successo!";
                db.Ordine.Add(ordine);
                db.SaveChanges();

                foreach(Cart item in ViewBag.Carrello)
                {
                    DettaglioOrdine d = new DettaglioOrdine(item.Quantita,item.IdAggiunzione,item.IdProdotto,ordine.IdOrdine);
